...
Mirrors the behavior of the original ``ml_models/twin_fwe/app/app.py`` so that
the website's twin-fwe page renders the same result view: percentile/z-score
...
def predict(payload: dict, submodule_root: str, static_root: str | None = None) -> dict:
    """Run the full twin-fwe pipeline.
...
def adjust_trend(trend_data_path: str, extended_by: int = 1) -> dict:
    """Re-render the trend-line plot with a new range and return template kwargs."""
    with open(trend_data_path, "rb") as fh:
        td = pickle.load(fh)
